...
fn test_normalize_envelope_preserves_tag_case_and_self_closing() {
    ...
    let expected = format!("<speak version=\"1.0\" xmlns=\"{SSML_XMLNS}\" xml:lang=\"en-US\"/>");
...
fn test_build_google_request_with_ssml_override() {
    ...
    let (body, words) = build_google_request("Hello world", "en-US-Wavenet-D", false, Some(ssml));
...
fn test_unrealspeech_config_matrix() {
    let cfg = build_config("unrealspeech", &engine_creds("unrealspeech")).expect("unrealspeech");
...
fn test_azure_ws_parse_word_boundary_with_text_offset() {
    ...
    let (word, _, _, char_offset, char_len) = azure_ws_parse_word_boundary(&item).expect("parsed");
...
fn test_azure_ws_parse_word_boundary_missing_data() {
    assert!(azure_ws_parse_word_boundary(&serde_json::json!({"Type": "WordBoundary"})).is_none());
}
...
fn test_streaming_chunk_size_used_in_speak_path() {
    ...
    for f in ["engine.rs", "decode.rs", "config.rs", "gemini.rs", "mux.rs"] {
        let p = std::path::Path::new("src/cloud_engine").join(f);
        source.push_str(&std::fs::read_to_string(&p).unwrap_or_default());
    }
    let production_uses = source
        .lines()
        .filter(|l| l.contains("STREAMING_CHUNK_SIZE"))
        .count();
    assert!(
        production_uses >= 2,
...
fn test_speechmarkdown_gemini_dialect_routing() {
    ...
    let (out, is_ssml) = preprocess_speech_markdown("Wait [500ms] then [laugh] loudly", "gemini");
