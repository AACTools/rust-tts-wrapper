...
fn interaction_json_no_audio() -> String {
    r#"{"steps":[{"type":"model_output","content":[{"type":"text","text":"I can't speak that sentence."}]}]}"#
        .to_string()
}
...
fn gemini_no_audio_block_is_an_error_with_detail() {
    let (port, handle) = spawn_mock(interaction_json_no_audio());
    let engine = gemini_engine(&format!("http://127.0.0.1:{port}/interactions"));
    ...
    assert!(
        ...
        "error mentions the missing audio: {msg}"
    );
}
