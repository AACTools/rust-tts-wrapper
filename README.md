...
- **Playback timeline** (`timeline` module): turn word boundaries into a `(playback_time → char_offset)` timeline for reader-style highlighting — `PlaybackTimeline` (built from `synth_with_boundaries` output or boundary-callback tuples; keyed in playback time so a tempo/speed factor is compensated) plus `PlaybackClock` (pause/resume/seek wall-clock). Binary-search lookups hold the last position across gaps. See `examples/timeline-demo.rs` for the full pattern.
